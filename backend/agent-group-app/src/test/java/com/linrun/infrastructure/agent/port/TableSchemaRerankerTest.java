...
class TableSchemaRerankerTest {
    ...
    private AgentTableSchemaMatch match(String modelCode, double score) {
        return new AgentTableSchemaMatch(modelCode, score, List.of());
    }

    @Test
    @DisplayName("按 rerank 分降序重排并更新 score")
    void shouldRerankByScoresDesc() {
        List<AgentTableSchemaMatch> matches = List.of(match("t1", 0.9), match("t2", 0.5), match("t3", 0.7));
        // rerank 给出的分数：t2 最高、t1 次之、t3 最低
        List<Double> scores = List.of(0.3, 0.95, 0.1);

        List<AgentTableSchemaMatch> reranked = TableSchemaReranker.rerank(matches, scores, 5);

        assertThat(reranked).extracting(AgentTableSchemaMatch::modelCode).containsExactly("t2", "t1", "t3");
    ...
    void shouldLimitByTopN() {
        ...
        List<AgentTableSchemaMatch> reranked = TableSchemaReranker.rerank(matches, scores, 2);
    ...
    void shouldFallbackWhenScoresMismatch() {
        List<AgentTableSchemaMatch> matches = List.of(match("t1", 0.9), match("t2", 0.5));
