...
## 项目背景与目标
面向内容生成与营销交易场景的综合项目，核心定位是 **多模式 `Agent`（智能体）工作台 + 拼团式额度交易平台**。项目两条主线分别是 `Agent`（智能体）智能执行和拼团交易营销，各约占项目表达的 40%，其余 20% 是运维、监控、支付沙箱、文档和演示支撑。

不是简单的聊天机器人，也不是单纯支付项目。`Agent`（智能体）侧重点在智能化执行能力：
...
拼团式额度交易平台是项目第二主线，不是附属小功能。它参考成熟拼团营销模型，把活动试算、锁单参团、组队成团、支付回调、额度到账、退款补偿和状态事件补偿改造成“优惠购买 `Agent`（智能体）调用额度”的业务链路。用户购买额度后，可以调用对话问答、文件理解、深度任务、`PPT`（演示文稿）生成、图像生成和技能编排等能力。

秋招表达时优先聚焦 **多模式 `Agent`（智能体）工作台与拼团式额度交易平台**，不要泛化成普通聊天机器人，也不要把拼团只说成页面玩法或支付附属功能。

## 当前技术定位

- 项目当前定位为：Java 后端工程能力 + Spring AI + **多模式 `Agent`（智能体）工作台** + **拼团式额度交易平台**。
- 核心特色：不是简单聊天机器人，而是**多模式执行引擎 + 智能重规划 + 反思评估 + 全链路追踪 + 拼团营销交易一致性**。
...
7. **拼团营销交易**：围绕额度包购买设计活动试算、锁单参团、组队成团、超时退款和成团发额
8. **交易状态一致性**：通过订单流水、额度流水、支付幂等键、Outbox 状态事件和定时补偿保证额度、订单、支付、拼团状态一致性

> 亮点中的百分比、成功率等数字须有单测或压测出处；路由准确率来自 `AgentEngineRoutingTest`，不要包装成线上 LLM 指标。

### 秋招项目表达

- 优先聚焦"**多模式 Agent 工作台 + 拼团式额度交易平台**"
- Agent 侧强调**智能化能力**：多模式、重规划、反思、追踪、诊断
- 拼团交易侧强调**营销交易能力**：活动试算、锁单参团、成团结算、发额规则、退款补偿、状态一致性
- 不要泛化成普通聊天机器人或单纯支付项目
