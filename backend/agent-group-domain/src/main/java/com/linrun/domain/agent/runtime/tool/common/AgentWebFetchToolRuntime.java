...
public class AgentWebFetchToolRuntime {
    ...
    private final AgentWebFetchPort remotePort;
    private final HttpClient httpClient;
    private final boolean allowPrivateAddresses;

    public AgentWebFetchToolRuntime() {
        this((AgentWebFetchPort) null);
    }

    public AgentWebFetchToolRuntime(AgentWebFetchPort remotePort) {
    ...
    public AgentWebFetchToolRuntime(HttpClient httpClient, boolean allowPrivateAddresses) {
    ...
    public static AgentToolDefinition definition() {
        return AgentToolDefinition.builder(AgentToolOutputNames.WEB_FETCH)
    ...
    public AgentToolStructuredOutput call(AgentToolCallCommand command) {
        ...
        if (remotePort != null) {
            AgentWebFetchPort.AgentWebFetchResult remoteResult = remotePort.fetch(
                    new AgentWebFetchPort.AgentWebFetchRequest(
                            requestId(command),
                            uri.toString(),
                            timeoutSeconds,
                            maxContentChars));
            if (remoteResult.success()) {
                return AgentToolStructuredOutput.builder(AgentToolOutputNames.WEB_FETCH)
    ...
    private HttpResponse<String> fetch(URI uri, int timeoutSeconds, int redirectCount)
        ...
        HttpRequest request = HttpRequest.newBuilder(checkedUri)
                ...
                .header("User-Agent", "agent-group-agent-web-fetch/1.0")
                .GET()
                .build();
    ...
    private String requestId(AgentToolCallCommand command) {
