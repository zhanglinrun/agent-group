...
3. 在前台运行论文精读、文件问答、`PPT`（演示文稿）大纲或图表重建任务，观察流式回答和额度消耗。
...
当前用户端已经接入登录注册、额度中心、学术 `Agent`（智能体）流式接口、文件上传、直接购买、拼团购买和模拟支付。购买入口直接基于额度包创建订单，额度是否到账以后端支付状态和拼团成团状态为准。

交易侧已经拆出关闭未支付订单、超时退款、通知补偿三类任务；通知配置由 `NotifyConfig`（通知配置值对象）承载，再通过 `TradeNotifyPort`（外部交易通知端口）派发。知识库侧已经补充向量入库失败标记、定时补偿任务和手动补偿接口。工具侧保留学术检索、文件理解、数据分析、`JSON`（结构化数据格式）修复和文档补偿能力；订单、退款和额度流水只作为后台排障数据。
