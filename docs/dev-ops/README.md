...
交易侧已经拆出关闭未支付订单、超时退款、通知补偿三类任务；通知配置由 `NotifyConfig`（通知配置值对象）承载，再通过 `TradeNotifyPort`（外部交易通知端口）派发。知识库侧已经补充向量入库失败标记、定时补偿任务和手动补偿接口。导购侧已经补充 `retrieval_progress`（检索进度）事件和查询路由，`MCP`（模型上下文协议）工具也暴露了查询路由、退款状态、`JSON`（结构化数据格式）修复和文档补偿能力。
