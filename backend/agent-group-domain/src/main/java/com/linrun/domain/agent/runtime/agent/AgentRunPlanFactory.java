...
public class AgentRunPlanFactory {

    public AgentPlan build(String taskType, boolean webSearchEnabled) {
        return switch (normalizeTaskType(taskType)) {
            ...
            case "data" -> dataPlan();
            case "trade-diagnosis" -> tradeDiagnosisPlan();
    ...
    private AgentPlan filePlan() {
        return new AgentPlan("文件问答", List.of(
    ...
    private AgentPlan pptPlan(boolean webSearchEnabled) {
        String materialStep = webSearchEnabled ? "搜索并整理主题资料" : "整理主题素材和结构";
        return new AgentPlan("PPT 生成", List.of(
    ...
    private AgentPlan deepResearchPlan(boolean webSearchEnabled) {
        String evidenceStep = webSearchEnabled ? "检索公开资料和参考来源" : "整理已有上下文和知识库信息";
        return new AgentPlan("深度任务", List.of(
                step("S1", "拆解任务问题、范围和风险点", 1, "规划智能体"),
    ...
    private AgentPlan imagePlan() {
        return new AgentPlan("图像生成", List.of(
    ...
    private AgentPlan dataPlan() {
        return new AgentPlan("数据问答", List.of(
    ...
    private AgentPlan tradeDiagnosisPlan() {
        return new AgentPlan("交易诊断", List.of(
                step("S1", "读取当前用户可见的订单、支付、退款和额度流水", 1, "交易诊断智能体"),
                step("S2", "对照支付成功、成团、退款和额度发放规则", 2, "交易校验智能体", "S1"),
                step("S3", "输出一致性结论、异常原因和处理建议", 3, "报告智能体", "S2")
        ));
    }

    private AgentPlan skillsPlan() {
        return new AgentPlan("技能执行", List.of(
    ...
    private AgentPlan manualSkillsPlan() {
        return new AgentPlan("手动技能执行", List.of(
    ...
    private AgentPlan chatPlan(boolean webSearchEnabled) {
        String secondStep = webSearchEnabled ? "按需检索或搜索补充信息" : "组织上下文和已有知识";
        return new AgentPlan("通用对话", List.of(
    ...
    private AgentPlanStep step(String stepId,
                                  String instruction,
                                  int order,
