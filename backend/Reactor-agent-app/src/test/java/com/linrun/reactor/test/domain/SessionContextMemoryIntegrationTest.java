...
import com.linrun.reactor.application.agent.execute.planexecute.PlanSolveAgentExecuteStrategy;
...
import com.linrun.reactor.application.agent.execute.react.ReactAgentExecuteStrategy;
import com.linrun.reactor.domain.agent.service.execute.react.step.factory.DefaultReactAgentExecuteStrategyFactory;
import com.linrun.reactor.infrastructure.reactor.service.impl.SessionContextMemoryServiceImpl;
...
public class SessionContextMemoryIntegrationTest {
    ...
    public void shouldConstructCasePrinterInsteadOfRequiringSseEmitter() throws Exception {
        ...
        Mockito.when(handler.apply(Mockito.any(AgentRequest.class), Mockito.any()))
                .thenAnswer(invocation -> {
                    ...
                    Assert.assertEquals("com.linrun.reactor.application.agent.stream.AgentSessionPrinter", printer.getClass().getName());
                    return "ok";
                });
        ...
        ReflectionTestUtils.setField(strategy, "defaultReactAgentExecuteStrategyFactory", factory);
        ReflectionTestUtils.setField(strategy, "reactorConfig", new ReactorConfig());
        ReflectionTestUtils.setField(strategy, "sessionContextMemoryService", Mockito.mock(com.linrun.reactor.domain.agent.memory.SessionContextMemoryService.class));
    ...
    private void seedSimpleHistory(ExecutionLedgerFixtureFactory.LedgerTestContext ctx,
                                   ...
                                   String sessionId,
                                   String thought) {
        Long runId = ctx.recorder.createRun(com.linrun.reactor.domain.agent.ledger.model.DialogueRunStartRecord.builder()
                .runUid(requestId)
                .requestId(requestId)
                .sessionId(sessionId)
                .entryAgent(com.linrun.reactor.domain.agent.ledger.model.ExecutionLedgerConstants.ENTRY_AGENT_REACT)
                .queryText("query:" + requestId)
                .startedAt(LocalDateTime.of(2026, 5, 4, 12, 0))
                .build());
        Long llmInvocationId = ctx.recorder.createLlmInvocation(com.linrun.reactor.domain.agent.ledger.model.LlmInvocationStartRecord.builder()
                .runId(runId)
                .requestId(requestId)
                .invocationSeq(1)
                .agentName("react")
                .stepNo(1)
                .callKind(com.linrun.reactor.domain.agent.ledger.model.ExecutionLedgerConstants.CALL_KIND_ASK_TOOL)
                .streaming(false)
                .modelName("test-model")
                .startedAt(LocalDateTime.of(2026, 5, 4, 12, 1))
                .build());
        ctx.recorder.finishLlmInvocation(com.linrun.reactor.domain.agent.ledger.model.LlmInvocationFinishRecord.builder()
                .llmInvocationId(llmInvocationId)
                .requestId(requestId)
                .status(com.linrun.reactor.domain.agent.ledger.model.ExecutionLedgerConstants.STATUS_SUCCESS)
                ...
                .build());
        ctx.recorder.finishRun(com.linrun.reactor.domain.agent.ledger.model.DialogueRunFinishRecord.builder()
                .runId(runId)
                .requestId(requestId)
                .status(com.linrun.reactor.domain.agent.ledger.model.ExecutionLedgerConstants.STATUS_SUCCESS)
