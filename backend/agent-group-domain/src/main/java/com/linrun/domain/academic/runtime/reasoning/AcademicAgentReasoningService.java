...
public class AcademicAgentReasoningService {
    ...
    private String inferTaskType(String question) {
        ...
        if (containsComplexAcademicIntent(lower)) {
            return "深度分析";
        }
    ...
    private int inferStepCount(String question) {
        ...
        String lower = question.toLowerCase();
        if (containsComplexAcademicIntent(lower) && !(containsSearchIntent(lower) && !containsStrongPlanningIntent(lower))) {
    ...
    private boolean containsComplexAcademicIntent(String lower) {
        boolean hasTaskAction = containsAny(lower, "研究", "调研", "分析", "综述", "梳理", "总结");
        boolean hasAcademicFocus = containsAny(lower, "对比", "比较", "差异", "优劣")
                || containsAny(lower, "现状", "趋势", "路线", "机制", "框架", "体系", "方案")
                || containsAny(lower, "问题", "风险", "挑战", "流程");
        return containsDeepResearchIntent(lower) || (hasTaskAction && hasAcademicFocus);
    }
