...
public final class PlanExecutePrompts {
    ...
    public static final String PLAN = """
            ...
            优先级最高：
            - 尤其需要关注最近一次的【Critique Feedback】提出的反馈意见，严格围绕按照他的意见，补充增量的执行计�?
            ...
            [
              {
                "id": "task-1",
                "instruction": "调用 <工具�? 工具，执�?<明确查询或操�?，获取XX结果",
                "order": 1
              },
              {
                "id": "task-2",
                "instruction": "根据task-1的执行结果，调用 <工具�? 工具，执�?<明确查询或操�?",
                "order": 2
              }
            ]

            示例4：具有先后关系的执行计划（并�?串行�?
            [
               {"id":"task-1","instruction":"调用 XXX 工具，执�?明确查询或操�?","order":1},
               {"id":"task-2","instruction":"调用 XXX 工具，执�?明确查询或操�?","order":1},
               {"id":"task-3","instruction":"根据 task1 �?task-2 的结果，调用 XXX 工具，执�?明确查询或操�?","order":2}
             ]
            """;
    ...
    public static final String EXECUTE = """
            ...
            - 如存在不确定或冲突信息，应如实保�?
            - 【Current Task】是你检索的唯一任务来源，不要偏离主�?
    ...
    public static final String CRITIQUE = """
            你是【DeepResearch 研究评审专家】�?
             ...
             1. 核心问题覆盖
             - 用户最关心的关键问题是否已有明确回应（足够较为全面的回答用户问题，满足百分�?0，则视为结束�?
             - 是否存在影响结论成立的关键缺�?
             - 部分敏感信息执行多次搜索都无法获取，则忽略此问题，可能无法搜索得到，不需要反复尝�?

             2. 证据可用�?
             - 当前已有事实和材料，是否足以支撑主要判断与结�?
             - 是否存在必须补充、否则结论无法成立的证据缺口

             输出要求（必须严格遵守）�?
             只允许输�?JSON，不得包含任何其他文字�?
    ...
    public static final String COMPRESS = """
             ...
             你的输出将直接作�?Agent 的下一轮上下文输入�?
             用于继续规划、判断和工具调用�?
             这是工作记忆压缩，不是给人类阅读的摘要�?

             ## 压缩目标
             将当前上下文压缩为：
             在不丢失关键信息的前提下，支�?Agent 下一轮正确决策的最小状态�?
    ...
    public static final String SUMMARIZE = """
            ...
            - **充分利用检索结�?*：工具检索到的事实、数据、结论是你回答的唯一依据
            ...
            ## 回答策略
            - 对于检索到的内容：详细、准确地呈现
            - 对于未检索到的内容：诚实说明"未检索到相关信息"�?基于现有信息无法判断"
    ...
    public static final String RESEARCH_TOPIC_GENERATION = """
            ...
            ## 任务目标
            基于用户的问题，列出需要研究的具体分析�?研究维度，为后续深度研究提供明确方向�?

            ## 重要说明
            - 你不需要去验证问题是否合理，也不需要去评判问题的真实�?
            - 你的任务是列出需要查询和分析的具体内�?
            - 输出应该是清晰的、可执行的研究要点列�?
            - 避免使用过于复杂或学术化的表�?

            ## 分析点组织原�?
            1. **基于用户意图**：从用户的问题中提取核心关注�?
            2. **拆解为具体查�?*：将模糊问题拆解为可搜索的具体内�?
            3. **多维度覆�?*：从不同角度列出需要了解的方面
            4. **简洁明�?*：每个分析点应该简单直接，易于理解和执�?
