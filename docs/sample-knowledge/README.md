...
网页端上传后，后端会保存到 `MinIO`（对象存储），解析文本，切分知识片段，并通过 `Spring AI VectorStore`（向量存储接口）写入 `pgvector`（向量库）。如果没有配置大模型密钥，导购会使用基于商品资料和知识片段的兜底回答，不再生成本地兜底向量。

评测用例样本在 `evaluation-cases.json`（评测用例配置），当前覆盖 50 个导购、拼团、售后、订单、支付风控和多轮追问场景。用例里包含期望工具顺序、期望关键词和禁答关键词，可以通过环境变量指定：
