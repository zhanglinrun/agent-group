...
## 项目背景与目标
本项目要实现一个带登录、额度、拼团和支付能力的 `AI Agent`（人工智能智能体）平台，核心路径是“登录注册-购买额度-使用额度-运行 Agent-记录消耗-支付与退款闭环”。

系统重点是把用户账号、额度账户、拼团订单、支付状态和 `Agent`（智能体）任务连接起来。用户通过直接购买或拼团购买额度，再用额度调用聊天、文件问答、`PPT`（演示文稿）、深度研究、技能等能力。
