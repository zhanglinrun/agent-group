...
class GuideRagPromptBuilderTest {
    ...
    void shouldBuildPromptWithProductAndReferences() {
        ...
        assertTrue(prompt.getSystemPrompt().contains("只基于额度包资料"));
        assertTrue(prompt.getUserPrompt().contains("拼团失败会退款吗"));
        assertTrue(prompt.getUserPrompt().contains("基础学术额度包"));
    ...
    static GuideDecisionResult decisionResult() {
        ...
        product.setSpecSummary("40 次普通学术问答额度");
        product.setAfterSalePolicy("未成团自动退款，已使用额度不退款");
        product.setRecommendReason("预算有限时普通学术问答性价比更高");
        ...
        reference.setDocumentType("退款规则");
        ...
        recommendationResult.addReason("AFTER_SALE_MATCH", "你关注退款，需要看成团状态和额度回滚规则。", 90);
    ...
    private static class FakePromptTemplateRepository implements PromptTemplateRepository {
        ...
        private final List<PromptTemplate> templates = List.of(
                PromptTemplate.enabled("PT-GUIDE-001", PromptTemplateType.GUIDE, "guide-v1.0", "只基于额度包资料回答。"),
