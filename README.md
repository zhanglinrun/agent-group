...
2. 决策快照下单：导购推荐会生成 `decisionId`（决策编号），下单时重新校验商品、活动、报价和过期时间，保证导购卡片、订单金额和支付单金额一致。
3. 工具型 `Agent`（智能体）：后端维护工具白名单，支持知识检索、商品推荐、拼团试算、订单查询等工具，流式返回工具计划和执行过程。
4. 电商交易闭环：支持拼团锁单、支付单创建、支付回调演示、订单状态流转、退款和补偿任务。
5. 知识库闭环：上传商品详情、活动规则、售后政策后，解析、切片、向量化并优先通过 `Spring AI VectorStore`（向量存储接口）写入 `pgvector`（向量库），文档原件保存到 `MinIO`（对象存储）。
6. 质量评测闭环：50 条评测用例覆盖检索命中、回答准确、推荐合理、多轮一致、工具调用、工具参数、工具结果引用和交易一致性。
7. 可观测性：接入 `Prometheus`（指标采集工具）和 `Grafana`（指标看板工具），观察工具调用、工具耗时、模型耗时、`Spring AI` 调用回退次数和向量检索状态。
...
2. 点击“拼团购买”，观察 `decisionId`（决策编号）校验、锁单、支付单、支付回调演示和订单状态流转。
...
基于 `Java 21`（后端语言）、`Spring Boot 3`（后端框架）和 `Spring AI`（Spring 人工智能框架）实现电商可信导购 `AI Agent`（人工智能智能体），将商品知识库、工具调用、导购决策快照、拼团交易、支付回调和离线评测串成闭环；通过工具白名单、报价重校验、检索依据展示、评测指标和监控看板降低大模型幻觉与交易金额不一致风险。
