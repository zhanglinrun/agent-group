...
public class AgentReflectionService {
     ...
     * 对计划执行结果进行反思。
     */
    public ReflectionResult reflect(AgentPlan plan, List<AgentPlanStep> completedSteps) {
        if (plan == null) {
            return ReflectionResult.empty();
        }

        List<AgentPlanStep> observedSteps = observedSteps(plan, completedSteps);
    ...
    private List<AgentPlanStep> observedSteps(AgentPlan plan, List<AgentPlanStep> completedSteps) {
        ...
        return plan.getSteps().stream()
                .filter(step -> !AgentPlanLifecycleService.STATUS_NOT_STARTED.equals(step.getStatus()))
                .toList();
    }
     ...
     * 评估计划执行质量
     */
    private double evaluatePlanQuality(List<AgentPlanStep> completedSteps) {
        ...
        long successfulSteps = completedSteps.stream()
                .filter(s -> AgentPlanLifecycleService.STATUS_COMPLETED.equals(s.getStatus()))
                .count();
     ...
     * 识别可改进点
     */
    private List<String> identifyImprovements(AgentPlan plan, List<AgentPlanStep> completedSteps) {
        List<String> improvements = new ArrayList<>();

        long failedSteps = completedSteps.stream()
                .filter(s -> AgentPlanLifecycleService.STATUS_BLOCKED.equals(s.getStatus()))
                .count();
        if (failedSteps > 0) {
