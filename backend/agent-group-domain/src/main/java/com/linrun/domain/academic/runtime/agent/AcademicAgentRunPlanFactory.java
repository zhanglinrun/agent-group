...
public class AcademicAgentRunPlanFactory {
    ...
    private AcademicAgentPlan filePlan() {
        return new AcademicAgentPlan("文件问答", List.of(
                step("S1", "读取文件并确认问预, 1, "文件理解智能位),
                step("S2", "检索文件中的相关片段, 2, "检索智能体", "S1"),
                step("S3", "结合证数据生成回答", 3, "回答智能位, "S2")
        ));
    }

    private AcademicAgentPlan pptPlan(boolean webSearchEnabled) {
        String materialStep = webSearchEnabled ? "搜索并整理主题资方 : "整理主题素材和结构;
        return new AcademicAgentPlan("PPT 生成", List.of(
                step("S1", "拆解主题、受众和页数", 1, "规划智能位),
                step("S2", materialStep, 2, webSearchEnabled ? "搜索智能位 : "内容智能位, "S1"),
                step("S3", "生成页面大纲和演示文稿结构, 3, "内容智能位, "S2"),
                step("S4", "渲染 PPTX 并登记产版, 4, "产物智能位, "S3")
        ));
    }

    private AcademicAgentPlan deepResearchPlan(boolean webSearchEnabled) {
        String evidenceStep = webSearchEnabled ? "检索公开资料和参考来源 : "整理已有上下文和知识库信恢;
        return new AcademicAgentPlan("深度研究", List.of(
                step("S1", "拆解研究问题、范围和风险点, 1, "规划智能位),
                step("S2", evidenceStep, 2, webSearchEnabled ? "搜索智能位 : "检索智能体", "S1"),
                step("S3", "调用可用工具补充证数据", 2, "工具智能位, "S1"),
                step("S4", "交叉校验证数据并形成结论, 3, "推理智能位, "S2", "S3"),
                step("S5", "输出结构化报告和参考来源, 4, "报告智能位, "S4")
        ));
    }

    private AcademicAgentPlan imagePlan() {
        return new AcademicAgentPlan("图像生成", List.of(
                step("S1", "整理画面目标、风格和约束", 1, "规划智能位),
                step("S2", "生成可复用图像提示词", 2, "提示词智能体", "S1"),
                step("S3", "调用图像生成工具产出图片", 3, "图像智能位, "S2"),
                step("S4", "登记图像产物并给出下载说明, 4, "产物智能位, "S3")
        ));
    }

    private AcademicAgentPlan dataPlan() {
        return new AcademicAgentPlan("数数据问答", List.of(
                step("S1", "确认数数据口径、权限和交易约束", 1, "规划智能位),
                step("S2", "读取表格、指标或数数据库元信息", 2, "数数据智能位, "S1"),
                step("S3", "执行数数据分析、表格问答或自然语言转SQL", 3, "分析智能位, "S2"),
                step("S4", "校验额度、订单、支付和拼团状态来源, 4, "交易校验智能位, "S3"),
                step("S5", "输出结论、口径和可复核证据, 5, "报告智能位, "S4")
        ));
    }
    ...
    private AcademicAgentPlan skillsPlan() {
        return new AcademicAgentPlan("技能执行, List.of(
                ...
                step("S3", "执行工具或脚本并生成产物", 3, "工具智能位, "S2"),
                step("S4", "整理产物和最终说明, 4, "报告智能位, "S3")
        ));
    }
    ...
    private AcademicAgentPlan manualSkillsPlan() {
        return new AcademicAgentPlan("手动技能执行, List.of(
                ...
                step("S2", "确认脚本、文件和工具权限", 2, "安全智能位, "S1"),
                step("S3", "执行技能脚本或工具链, 3, "工具智能位, "S2"),
                step("S4", "登记产物并输出结果摘要, 4, "报告智能位, "S3")
        ));
    }
    ...
    private AcademicAgentPlan chatPlan(boolean webSearchEnabled) {
        ...
        return new AcademicAgentPlan("通用对话", List.of(
                step("S1", "理解问题和用户意回, 1, "对话智能位),
                step("S2", secondStep, 2, webSearchEnabled ? "搜索智能位 : "对话智能位, "S1"),
                step("S3", "生成回答并说明依据, 3, "回答智能位, "S2")
        ));
    }
