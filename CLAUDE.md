...
面向内容生成与营销交易场景的 **多模式 `Agent`（智能体）工作台与拼团式额度交易平台**，不是普通聊天机器人，也不是单纯支付项目。项目两条主线是 `Agent`（智能体）智能执行和拼团交易营销，各约占项目表达的 40%，其余 20% 是运维、监控、支付沙箱、文档和演示支撑。后端是 `Java 21` + `Spring Boot 3` + `Spring AI` 的 `Maven`（构建工具）多模块工程，前端是 `React 19` + `Vite`。
...
拼团交易是项目第二主线，不是附属小功能。它把传统“优惠买商品”的拼团营销玩法改造成“优惠买 Agent 调用额度”，重点讲活动试算、锁单参团、组队成团、支付回调、额度到账、退款补偿和状态一致性。
...
- 秋招表达时可以讲 DDD 分层、锁单幂等、Redis 分布式锁、库存和队伍名额占位、成团结算、超时退款和补偿任务；当前工程是 Maven 多模块单体启动，不要直接写成已落地微服务集群。
