...
/**
 * Agent 的会话管理服务，从 AgentNativeService 抽出。
 * 负责会话查询、消息回放、删除、按消息回滚、停止运行，以及确定性提问落库和 agentType 回填。
 */
@Service
public class AgentSessionService {

    private static final Logger LOGGER = LoggerFactory.getLogger(AgentSessionService.class);
    ...
    public AgentSessionService(AiSessionService sessionService,
    ...
    public void deleteSession(String token, String conversationId) {
        ...
        for (FileInfo file : relatedFiles) {
            try {
                ...
                LOGGER.warn("agent-runtime session file cleanup degraded, fileId={}, reason={}", file.getFileId(), e.getClass().getSimpleName());
