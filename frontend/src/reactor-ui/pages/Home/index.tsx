...
import UserWorkspaceToolbar from "@/components/UserWorkspaceToolbar";
...
const Home: ReactorType.FC<HomeProps> = memo(() => {
  ...
  const [accountAuth, setAccountAuth] = useState(() => getUserAuth());
  ...
  if (!accountAuth?.token) {
    return (
      <AccountAuthGate
        onAuthenticated={() => setAccountAuth(getUserAuth())}
      />
    );
  }
    ...
    <div className="h-full w-full bg-[var(--page-gradient)] text-foreground">
      <div className="flex h-full w-full">
        <ConversationSidebar
          ...
          onAccountLogout={() => setAccountAuth(null)}
        />
        ...
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <div className="border-b border-[var(--chat-border)] bg-[var(--chat-surface)]/80 px-4 py-3 backdrop-blur-md sm:px-6">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <UserWorkspaceToolbar className="lg:ml-4" />
            </div>
          </div>
