...
VALUES (1, 'agent-group-trade-job', 'agent-group交易补偿执行器', 0, NULL, NOW());
...
VALUES (101, 1, '交易事件Outbox投递', NOW(), NOW(), 'agent-group', '',
       ...
       (102, 1, '拼团业务通知重试', NOW(), NOW(), 'agent-group', '',
        ...
        'BEAN', '', 'init', NOW(), ''),
       (103, 1, '超时未成团退款补偿', NOW(), NOW(), 'agent-group', '',
        'CRON', '0 */1 * * * ? *', 'DO_NOTHING', 'FIRST',
        'timeoutGroupRefundJobHandler', '', 'SERIAL_EXECUTION', 120, 1,
        'BEAN', '', 'init', NOW(), ''),
       (104, 1, '支付查单与未支付关单补偿', NOW(), NOW(), 'agent-group', '',
        'CRON', '0 0/5 * * * ? *', 'DO_NOTHING', 'FIRST',
        'paymentQueryCompensationJobHandler', '', 'SERIAL_EXECUTION', 120, 1,
        'BEAN', '', 'init', NOW(), '');
