...
import com.linrun.domain.academic.runtime.agent.AcademicPlanLifecycleResult;
...
public class AcademicPlanningToolRuntime {
    ...
    private final AcademicPlanLifecycleService lifecycleService;
    private final AcademicAgentFlowProjector flowProjector;
    private AcademicAgentPlan currentPlan;

    public AcademicPlanningToolRuntime() {
        this(new AcademicPlanLifecycleService(), new AcademicAgentFlowProjector());
    }

    public AcademicPlanningToolRuntime(AcademicPlanLifecycleService lifecycleService,
                                       AcademicAgentFlowProjector flowProjector) {
        this.lifecycleService = lifecycleService == null ? new AcademicPlanLifecycleService() : lifecycleService;
    ...
    public static AcademicToolDefinition definition() {
        return AcademicToolDefinition.builder(AcademicToolOutputNames.PLANNING)
    ...
    public synchronized AcademicToolStructuredOutput call(AcademicToolCallCommand command) {
        Map<String, Object> arguments = command == null ? Map.of() : command.getArguments();
        String operation = text(arguments.get("command"));
        AcademicPlanLifecycleResult result = switch (operation) {
    ...
    public synchronized AcademicAgentPlan currentPlan() {
        return currentPlan == null ? null : currentPlan.copy();
    }

    private AcademicPlanLifecycleResult create(Map<String, Object> arguments) {
        return lifecycleService.create(text(arguments.get("title")), stringList(arguments.get("steps")));
    }

    private AcademicPlanLifecycleResult update(Map<String, Object> arguments) {
        requireCurrentPlan();
        return lifecycleService.updateRemaining(currentPlan, text(arguments.get("title")), stringList(arguments.get("steps")));
    }

    private AcademicPlanLifecycleResult markStep(Map<String, Object> arguments) {
        requireCurrentPlan();
    ...
    private AcademicPlanLifecycleResult finish() {
        return lifecycleService.finish(currentPlan);
    }

    private AcademicPlanLifecycleResult snapshot() {
        requireCurrentPlan();
        return lifecycleService.ensureExecutable(currentPlan);
    }

    private AcademicToolStructuredOutput output(String command, AcademicPlanLifecycleResult result) {
        AcademicAgentPlan plan = result.getPlan();
        ...
        return AcademicToolStructuredOutput.builder(AcademicToolOutputNames.PLANNING)
    ...
    private Map<String, Object> stepMap(AcademicPlanStep step) {
    ...
    private Map<String, Object> stageMap(AcademicAgentFlowStage stage) {
        Map<String, Object> map = new LinkedHashMap<>();
