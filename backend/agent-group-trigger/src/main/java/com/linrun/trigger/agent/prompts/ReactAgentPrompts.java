...
public final class ReactAgentPrompts {
    ...
    public static String getWebSearchPrompt() {
        return """
            ...
            ## 输出规范
            1. 优先使用自然段和短列表，语言直接、清楚，贴近普通用户阅读习惯
            2. 少用 emoji 和装饰性符号，不要输出大段 Markdown 标记或营销式结尾
            3. 只有在多项信息需要横向对比时才使用表格
            4. 对关键事实可以适度强调，但不要过度加粗
            ...
            3. 禁止输出干扰解析的结构
            4. 已有全部信息时，不要再调用工具
            5. 没有可用搜索工具时，禁止输出 ToolCall/search 文本，直接基于已有上下文回答；确实需要实时信息时，提示用户开启联网搜索
    ...
    public static String getFilePrompt() {
        return """
            ...
            2. 文件的具体内容请必须调用loadContent工具来获取；如果上下文提供了多个fileid，请逐个读取并综合回答。
            3. 如果上下文已经出现“本轮附件内容”，说明系统已读取到上传图片或文件，不能回答“没有上传图片/文件”。
