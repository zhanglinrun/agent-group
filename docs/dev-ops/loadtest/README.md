...
- `report/index.html` 里有吞吐量、P90/P95/P99 响应时间和错误率，结果数字以这里为准；`report/` 和 `result*.jtl` 是本地生成物，不提交到 Git。
- 压测同时可以打开 `Grafana`（监控面板，docker 环境默认 13000 端口）看"Agent 运行与大模型调用监控"和工具/支付面板，观察服务端视角的延迟和错误。
...
- 本仓库只保留压测脚本和记录模板，不保留历史压测报告。需要对外引用数据时，重新跑压测并把环境、参数、结果写入上面的表格。
...
- 支付回调（`/api/v1/trade/payment/webhook`）涉及验签和防重放，不适合直接用本脚本压；如需评估，单独构造带验签的请求。
