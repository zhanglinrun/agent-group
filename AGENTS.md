# AGENTS.md instructions for N:\java_project\agent-group
...
## 项目推进规划
本项目按“小版本、小能力、小提交”推进，参考 `group-buy-market`（拼团参考项目）的提交节奏。整体规划约 34 个提交，当前前 5 个已完成。提交名保持简短，延续前五个提交的风格，不再写“第几课”。
...
4. `feat: 接口对象`（已完成）
5. `feat: 流式接口`（已完成）
6. `feat: 运行基线整理`：修正中文编码、统一 `Java 21`（二十一版 Java）、补启动配置。
7. `feat: 前端页面接入准备`：整理 `frontend`（前端目录）静态页，把乱码文案修正，保留用户端和运营端结构。
