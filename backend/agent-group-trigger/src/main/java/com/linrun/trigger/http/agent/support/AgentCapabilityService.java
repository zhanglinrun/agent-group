...
/**
 * Agent 的能力展示服务，从 AgentNativeService 抽出。
...
public class AgentCapabilityService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AgentCapabilityService.class);
    ...
    public Map<String, Object> capabilities() {
        ...
        List<Map<String, Object>> agentTools = agentToolCallbackFactory == null
                ? List.of()
                : agentToolCallbackFactory.preview("capabilities", true);
        int offlineAgentToolCount = agentToolCallbackFactory == null
                ? 0
                : agentToolCallbackFactory.preview("capabilities", false).size();
        ...
        result.put("agentToolAvailable", !agentTools.isEmpty());
        result.put("agentToolCount", agentTools.size());
        result.put("agentOfflineToolCount", offlineAgentToolCount);
        result.put("agentTools", agentTools);
        ...
        result.put("manualSkillsEndpoint", "/api/v1/agent/stream");
        ...
        List<Map<String, Object>> workspaceProfiles = workspaceProfiles(agentTools);
        List<Map<String, Object>> toolRuntimeReadiness = toolRuntimeReadiness(agentTools, workspaceProfiles);
        ...
        List<Map<String, Object>> capabilityMatrix =
                capabilityMatrix(agentTools, mcpAdminHealth, agentAdminStatistics,
        ...
        result.put("toolCatalog", toolCatalog(agentTools, workspaceProfiles));
        ...
        return List.of(
                ...
                agentExecutionMode("deep", "深度任务", "plan-execute", "Plan-Execute", "计划拆解、分步执行、反思评估和动态重规划",
                        ...
                        List.of("plan_update/replan stream event",
                                "AgentFlowProgress.STATUS_REPLANNED",
                                "AgentFallbackReplanStrategy default recovery",
    ...
    private List<Map<String, Object>> workspaceProfiles(List<Map<String, Object>> agentTools) {
        List<String> toolNames = toolNames(agentTools);
        return List.of(
                ...
                workspaceProfile("image", "/workspace/image", "image", "file-or-image",
                        ...
                        "/api/v1/agent/workspaces/image/generate",
                        "/api/v1/agent/workspaces/image/history",
                        toolNames),
                workspaceProfile("data", "/workspace/data", "data", "file",
                        ...
                        "/api/v1/agent/workspaces/data/run",
                        "/api/v1/agent/workspaces/data/history",
                        toolNames),
                workspaceProfile("trade", "/workspace/trade", "trade-diagnosis", "none",
                        ...
                        "/api/v1/agent/stream", "/api/v1/trade/order/my", toolNames)
        );
    }
    ...
    private Map<String, Object> toolCatalog(List<Map<String, Object>> agentTools,
        ...
        for (Map<String, Object> tool : agentTools == null ? List.<Map<String, Object>>of() : agentTools) {
        ...
        catalog.put("total", agentTools == null ? 0 : agentTools.size());
    ...
    private List<Map<String, Object>> capabilityMatrix(List<Map<String, Object>> agentTools,
        ...
        return List.of(
                ...
                capabilityItem("tool-runtime", "工具运行时", "ready",
                        ...
                        mergeEvidence(implementedTools, List.of("AgentToolRuntimeSummary 统计 total/enabled/disabled/category/source")),
                        missingRuntimeTools),
    ...
    private List<Map<String, Object>> toolRuntimeReadiness(List<Map<String, Object>> agentTools,
                                                           List<Map<String, Object>> workspaceProfiles) {
        List<String> availableToolNames = toolNames(agentTools);
        ...
        return AgentToolOutputNames.orderedRichToolNames().stream()
                .map(toolName -> {
    ...
    private List<Map<String, Object>> toolRuntimeFamilies(List<Map<String, Object>> toolRuntimeReadiness) {
        ...
        return List.of(
                toolRuntimeFamily("web", "网页抓取", List.of(AgentToolOutputNames.WEB_FETCH, AgentToolOutputNames.DEEP_SEARCH), byName),
                toolRuntimeFamily("data", "数据分析", List.of(AgentToolOutputNames.DATA_ANALYSIS, AgentToolOutputNames.TABLE_RAG, AgentToolOutputNames.NL2SQL), byName),
                toolRuntimeFamily("image", "图像生成", List.of(AgentToolOutputNames.IMAGE_GENERATION), byName),
                toolRuntimeFamily("report", "报告工具", List.of(AgentToolOutputNames.REPORT_TOOL), byName),
                toolRuntimeFamily("code", "代码解释器", List.of(AgentToolOutputNames.CODE_INTERPRETER, AgentToolOutputNames.SCRIPT_RUNNER), byName),
                toolRuntimeFamily("multimodal", "多模态", List.of(AgentToolOutputNames.MULTIMODAL_AGENT, AgentToolOutputNames.FILE_TOOL), byName)
        );
    }
    ...
    private Map<String, Object> agentPlatformReadiness(List<Map<String, Object>> executionModes,
        ...
        long replanModeCount = executionModes.stream()
                ...
                .count();
        List<String> orderedTools = AgentToolOutputNames.orderedRichToolNames();
    ...
    private List<String> fallbackInputFields(String toolName) {
        return switch (toolName) {
            case AgentToolOutputNames.WEB_FETCH -> List.of("url", "query");
            case AgentToolOutputNames.DATA_ANALYSIS -> List.of("task", "rows", "columns");
            case AgentToolOutputNames.REPORT_TOOL -> List.of("title", "content", "format");
            case AgentToolOutputNames.PLANNING -> List.of("goal", "context");
            case AgentToolOutputNames.CODE_INTERPRETER -> List.of("task", "language", "code");
            case AgentToolOutputNames.IMAGE_GENERATION -> List.of("prompt", "model", "quality", "aspectRatio", "size", "mode");
            case AgentToolOutputNames.MULTIMODAL_AGENT -> List.of("question", "imageUrls", "fileIds");
            case AgentToolOutputNames.DEEP_SEARCH -> List.of("query", "scope");
            case AgentToolOutputNames.FILE_TOOL -> List.of("action", "fileId", "fileName");
            case AgentToolOutputNames.SCRIPT_RUNNER -> List.of("scriptName", "runtime", "arguments");
            case AgentToolOutputNames.TABLE_RAG -> List.of("query", "modelCodeList");
            case AgentToolOutputNames.NL2SQL -> List.of("query", "schemaInfo");
            default -> List.of();
        };
    }
    ...
    private List<String> outputKindsForTool(String toolName) {
        return switch (toolName) {
            case AgentToolOutputNames.WEB_FETCH -> List.of("web", "file");
            case AgentToolOutputNames.DATA_ANALYSIS -> List.of("table", "summary");
            case AgentToolOutputNames.REPORT_TOOL -> List.of("report", "artifact");
            case AgentToolOutputNames.PLANNING -> List.of("plan", "flow");
            case AgentToolOutputNames.CODE_INTERPRETER -> List.of("code", "file", "summary");
            case AgentToolOutputNames.IMAGE_GENERATION -> List.of("image", "artifact");
            case AgentToolOutputNames.MULTIMODAL_AGENT -> List.of("multimodal", "evidence");
            case AgentToolOutputNames.DEEP_SEARCH -> List.of("answer", "reference");
            case AgentToolOutputNames.FILE_TOOL -> List.of("file", "content");
            case AgentToolOutputNames.SCRIPT_RUNNER -> List.of("script", "artifact");
            case AgentToolOutputNames.TABLE_RAG -> List.of("schema", "evidence");
            case AgentToolOutputNames.NL2SQL -> List.of("sql", "table");
    ...
    private String toolRuntimeHint(String toolName) {
        ...
        return switch (toolName) {
            case AgentToolOutputNames.IMAGE_GENERATION -> base + " Image generation also requires model and storage config in reactor-tool .env.";
            case AgentToolOutputNames.DATA_ANALYSIS,
                 AgentToolOutputNames.TABLE_RAG,
                 AgentToolOutputNames.NL2SQL -> base + " Data tools require table, database, or vector retrieval config.";
            case AgentToolOutputNames.DEEP_SEARCH,
                 AgentToolOutputNames.WEB_FETCH -> base + " Search tools require an external search provider; direct search is used as fallback.";
            case AgentToolOutputNames.MULTIMODAL_AGENT -> base + " Multimodal tools require file parsing, table understanding, and image understanding capability.";
            case AgentToolOutputNames.SCRIPT_RUNNER,
                 AgentToolOutputNames.CODE_INTERPRETER -> base + " Script tools require a local Python/code execution runtime.";
            default -> base;
        };
    }
    private String categoryForTool(String toolName) {
        return switch (toolName) {
            case AgentToolOutputNames.WEB_FETCH -> "web";
            case AgentToolOutputNames.DEEP_SEARCH -> "search";
            case AgentToolOutputNames.DATA_ANALYSIS,
                 AgentToolOutputNames.TABLE_RAG,
                 AgentToolOutputNames.NL2SQL -> "data";
            case AgentToolOutputNames.REPORT_TOOL -> "report";
            case AgentToolOutputNames.PLANNING -> "planning";
            case AgentToolOutputNames.CODE_INTERPRETER -> "code";
            case AgentToolOutputNames.IMAGE_GENERATION -> "image";
            case AgentToolOutputNames.MULTIMODAL_AGENT -> "multimodal";
            case AgentToolOutputNames.FILE_TOOL -> "file";
            case AgentToolOutputNames.SCRIPT_RUNNER -> "skill";
            default -> "tool";
        };
    }
    ...
    private Map<String, Object> capabilityItem(String key,
        ...
        if ("multi-agent".equals(key)) {
            ...
            dynamicReplan.put("historyEvidence", List.of("AgentReplayProjector", "planner history versions", "AgentFallbackReplanStrategy"));
